...
// QUESTIONS SOLVED IN IT
// Reverse a linked list
// middle of linked list
// Sorted Insert
// nth Node from End
// Remove Duplicats of a linked list
// Detect Loop in a LL
struct node
{
    int data;
...
int middleOfLinkedList(struct node *head, int listsize)
{
    struct node *current = head;
    if (head == NULL)
        return 0;
...
int middleOfLinkedListSlowFastMethod(struct node *head)
{
    struct node *current = head;
...
int nthNodeFromEnd(struct node *head, int pos)
{
    struct node *current = head;
    int pindex = listSize(head) - pos + 1;
    if (pindex < 0 || pindex > listSize(head))
        return 0;
    else
    {

        for (int p = 1; p < pindex; p++)
        {
            current = current->next;
        }
        return current->data;
...
int nthNodeFromEndTwoPointer(struct node *head, int pos)
{
...
struct node *reverseNAIVE(struct node *head)
{
...
int main()
{
    ...
    // cout << nthNodeFromEndTwoPointer(head, 3) << endl;

    // head = sortedInsert(head, 7);
    // cout << "list after sorted insert " << endl;
    // plist(head);

    // int listsize = listSize(head);
    // cout << middleOfLinkedList(head, listsize)<<endl;
    // cout << middleOfLinkedListSlowFastMethod(head)<<endl;
